"""
[PH]: This program builds upon "singletagOSC.py", extracting Position Data (x, y, z) and Orientation Data [now in Quarternions] (x, y, z, w).
...
class ReadyToLocalize(object):
    ...
    def printPublishPosition(self, position):
        ...
        print("POS ID {}, x(mm): {pos.x} y(mm): {pos.y} z(mm): {pos.z}, orientation: {ori[0]} {ori[1]} {ori[2]} {ori[3]}".format(
            "0x%0.4x" % network_id, pos=position, ori=orientationValues))

        # Transmit output to OSC
        if self.osc_udp_client is not None:
            self.osc_udp_client.send_message("/position", [network_id, int(position.x), int(position.y), int(position.z), orientationValues[0], orientationValues[1], orientationValues[2], orientationValues[3]])
    ...
    #[PH]: Extract Orientation data from Pozyx sensor [in Quarternions: x, y, z, w]
    def GetOrientation(self):
        ...
        x = float(orientationData[0].lstrip("X: "))
        y = float(orientationData[1].lstrip("Y: "))
        z = float(orientationData[2].lstrip("Z: "))
        w = float(orientationData[3].lstrip("W: "))
...
if __name__ == "__main__":
    ...
    '''************************************************************************************************************************************************'''
    # Set Remote ID
    tag_id = 0x6720                   # Pozyx Tag ID (in hexadecimal)
    ...
    anchors = [DeviceCoordinates(0x675a, 1, Coordinates(0, 0, 1175)),
               DeviceCoordinates(0x674d, 1, Coordinates(3500, -2330, 2110)),
