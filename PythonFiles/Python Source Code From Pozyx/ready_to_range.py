...
if __name__ == "__main__":
    ...
    # serial_port = 'COM11'        # [PH] Manually set to COM7 but may need to be changed if user is running on different local machine
    ...
    remote_id = 0x674d           # eg: id of an anchor not connected to the local machine (remotely connected)
    
    remote = False               # true: use a remote device for ranging, false: use a locally connected device for ranging (serial connection via USB)
    if not remote:
        remote_id = None

    # network ID of the ranging destination
    destination_id = 0x6720      # [PH]: This is the id of the tag (remote connection)!  May be changed depending on which tag is connected

    # distance that separates the amount of LEDs lighting up.
    range_step_mm = 20          # [PH] was 1000mm (1m)
